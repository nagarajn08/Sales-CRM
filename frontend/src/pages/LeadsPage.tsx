                ...
                {leads.map((lead) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 hidden sm:table-cell text-muted-foreground" onClick={() => navigate(`/leads/${lead.id}`)}>
                        <p className="font-data text-xs">{lead.mobile ?? "—"}</p>
                        {lead.email && <p className="text-xs mt-0.5 truncate max-w-[140px]">{lead.email}</p>}
                      </td>
