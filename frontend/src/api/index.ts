...
export const authApi = {
  ...
  otpRequest: (data: { email: string; mobile: string }) =>
    api.post<{ detail: string; email_sent: boolean; smtp_configured: boolean; email_otp_enabled: boolean; mobile_otp_enabled: boolean; dev_email_otp: string | null; dev_mobile_otp: string | null }>("/api/auth/otp/request", data).then(r => r.data),
...
export const settingsApi = {
  ...
  testSmtp: (data?: { smtp_host: string; smtp_port: string; smtp_user: string; smtp_password: string }) =>
    api.post<{ ok: boolean; detail: string }>("/api/settings/smtp-test", data ?? {}).then(r => r.data),
};
